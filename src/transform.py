...
def _drop_internal_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_cleaned = df.drop(columns=internal_cols)
    print(f"Dropped {len(internal_cols)} internal columns")
...
def transform_products() -> pd.DataFrame:
    ...
    engine = get_engine()
    df.to_sql("dim_products", engine, schema=SILVER_SCHEMA)
...
def transform_users() -> pd.DataFrame:
    ...
    engine = get_engine()
    df.to_sql("dim_users", engine, schema=SILVER_SCHEMA)
...
def transform_orders() -> pd.DataFrame:
    ...
    engine = get_engine()
    df.to_sql("fct_orders", engine, schema=SILVER_SCHEMA)
...
def transform_order_line_items() -> pd.DataFrame:
    ...
    diff = df["line_total_usd"] - expected_total
    bad_rows = df[diff.abs() > 0.01]
    print(f"Found {len(bad_rows)} rows with invalid line_total_usd")

    # TODO: Step 4 — Load into Silver as "fct_order_lines"
    engine = get_engine()
    df.to_sql("fct_order_lines", engine, schema=SILVER_SCHEMA, if_exists="replace", index=False)
